...
func main() {
	...
	for _, v := range []struct {
		...
		{"ceid-timeout-queue", 1100 * time.Millisecond, crawlExecutionTimeoutWorker(db)},
		{"remuri", 200 * time.Millisecond, removeUriQueueWorker(db)},
		{"busy-queue", 50 * time.Millisecond, busyQueueWorker(db)},
		{"wait-queue", 50 * time.Millisecond, waitQueueWorker(db)},
		{"ceid-running-queue", 50 * time.Millisecond, executionTimeoutQueueWorker(db)},
	} {
		t := v
